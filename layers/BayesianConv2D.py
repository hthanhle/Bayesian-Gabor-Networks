...
print('Bayesian Conv2D by original paper')
...
class BayesianConv2D(nn.Module):
    ...
    def forward(self, x, sample=True):
        ...
        # Local reparameterization trick for convolutional layers. See Eq. 4 in the paper 'Uncertainty estimation by softplus normalization...'
        # Compute 2 sequential convolutions (for the mean and the variance).

        # The first term in Eq. 4: x * W_mu
        act_mu = F.conv2d(
            x, self.W_mu, self.bias_mu, self.stride, self.padding, self.dilation, self.groups)

        # The second term in Eq. 4: sqrt(x^2 * W_sigma^2). The notations in the paper are really confusing.
    ...
    # In[4]: Compute KL divergence loss
    def kl_loss(self):
