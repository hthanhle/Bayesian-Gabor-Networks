...
print('Bayesian Gabor v3 with updated prior')
...
class BayesianGabor2D(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size,
        ...
        self.psi_mu = Parameter(torch.empty((out_channels,
                                             in_channels)))  # Note that we do not need to move the tensors manually to the device, since they follow the model cuda
    ...
    def forward(self, input, sample=True):
        ...
        eps = self.eps.normal_(0,
                               1)  # Note that we just need to replace the values with the samples drawn from the normal distrubution (a trick used to avoid duplicating the initialzization)

        # Draw random samples from the variational porterior distributions (i.e. weight = mu + eps * sigma)
        self.psi_sigma = torch.log1p(torch.exp(
            self.psi_rho))  # Sofplus function. Note that 'sigma_sigma' denotes the deviation of the variational distribution of the parameter 'sigma'
        self.psi = (
                    self.psi_mu + eps * self.psi_sigma).float()  # 'sigma' is a sample drawn from the distribution. Its shape is identical to the shape of 'sigma_mu' and 'sigma_rho'.
