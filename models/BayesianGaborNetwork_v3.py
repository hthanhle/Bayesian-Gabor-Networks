...
from layers.BayesianGabor_v2 import BayesianGabor2D
...
# This is a modified version of BayesianGaborPFPN_config3, where the number of kernel in every Bayesian Gabor layers is halved
print('BayesianGaborNetwork-v3')
...
class BayesianGaborNetwork(nn.Module):
    def __init__(self, num_inputs, num_classes, priors, activation_type='relu'):
        ...
        self.gabor1 = BayesianGabor2D(num_inputs, 16, 15, padding=7, priors=self.priors)
        self.act1 = self.act()
        self.down0 = nn.MaxPool2d(kernel_size=2, stride=2)
        self.gabor2 = BayesianGabor2D(16, 16, 15, padding=7, priors=self.priors)
        self.act2 = self.act()
        self.down1 = nn.MaxPool2d(kernel_size=2, stride=2)
        self.gabor3 = BayesianGabor2D(16, 16, 15, padding=7, priors=self.priors)
        self.act3 = self.act()

        self.down2 = nn.MaxPool2d(kernel_size=2, stride=2)
        self.gabor4 = BayesianGabor2D(16, 32, 7, padding=3, priors=self.priors)
        self.act4 = self.act()
        self.down3 = nn.MaxPool2d(kernel_size=2, stride=2)
        self.gabor5 = BayesianGabor2D(32, 32, 7, padding=3, priors=self.priors)
        self.act5 = self.act()

        self.down4 = nn.MaxPool2d(kernel_size=2, stride=2)
        self.gabor6 = BayesianGabor2D(32, 32, 5, padding=2, priors=self.priors)
        self.act6 = self.act()
        self.down5 = nn.MaxPool2d(kernel_size=2, stride=2)
        self.gabor7 = BayesianGabor2D(32, 32, 5, padding=2, priors=self.priors)
        self.act7 = self.act()
        self.up1 = nn.Upsample(scale_factor=16, mode='bilinear', align_corners=True)
        self.gabor8 = BayesianGabor2D(32, 64, 3, padding=1, priors=self.priors)
        self.act8 = self.act()

        self.gabor9 = BayesianGabor2D(32, 32, 5, padding=2, priors=self.priors)
        self.act9 = self.act()
        self.up2 = nn.Upsample(scale_factor=4, mode='bilinear', align_corners=True)
        self.gabor10 = BayesianGabor2D(32, 64, 3, padding=1, priors=self.priors)
        self.act10 = self.act()

        self.gabor11 = BayesianGabor2D(16, 32, 5, padding=2, priors=self.priors)
        self.act11 = self.act()
        self.gabor12 = BayesianGabor2D(32, 64, 3, padding=1, priors=self.priors)
        self.act12 = self.act()

        self.up3 = nn.Upsample(scale_factor=4, mode='bilinear', align_corners=True)
        self.gabor13 = BayesianGabor2D(64, 16, 3, padding=1, priors=self.priors)
        self.act13 = self.act()
        self.gabor14 = BayesianGabor2D(16, self.num_classes, 1, padding=0, priors=self.priors)
