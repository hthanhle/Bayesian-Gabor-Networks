...
import yaml
from utils.compute_ece_ause_numpy import ECEHelper
...
def test_model(model, test_loader):
    ...
    for i, (img, gt) in enumerate(tqdm(test_loader, ncols=80, desc='Testing')):
        ...
        start = timer()
        logits = model(img)  # Tensor shape: [batch_size, num_classes, H, W]
        end = timer()
        inference_time.append(end - start)

        pred_score = logits.squeeze(0)  # remove the first dimension of 1
        ...
        accs.append(np.mean(seg_map == gt))
        ious.append(jaccard_score(gt.flatten(), seg_map.flatten(), average='macro'))
        f_measures.append(f1_score(gt.flatten(), seg_map.flatten(), average='macro'))

        # Compute ECE
        ece_helpers.distribute_to_bins(logits=logits.cpu().detach().numpy(), labels=gt)  # Accumulate the values

    return np.mean(accs), np.mean(ious), np.mean(f_measures), np.mean(
        inference_time[10:]), ece_helpers.get_ece()  # skip several first predictions so that the inference time becomes stable
...
if __name__ == '__main__':
    ...
    # Evaluate the model
    acc, miou, f1, time, ece = test_model(model, test_loader)

    print('Accuracy: {}. mIoU: {}. F-measure: {}. Inference time: {}. ECE: {}'.format(acc, miou, f1, time, ece))
