...
This version accepts the inputs as Numpy arrays. The aim is to work with both Keras code and Pytorch code
@author: Hieu Phan
"""
...
# In[1]: Define a class Bin for ECE computation
class Bin:
...
class ECEHelper:
    ...
    def distribute_to_bins(self, logits, labels):
        """
        'logits' is the outputs before softmax, expected shape: [batch_size, num_classes, H, W]. For testing, batch size must be 1.
        'labels' is the groundtruth, expected shape: [batch_size, H, W].
        """
...
class AUSEHelper():
    ...
    def store_values(self, logits, labels, uncertainty):
        ...
        'logits' is the outputs before softmax, expected shape: [batch_size, num_classes, H, W]. For testing, batch size must be 1.
        'labels' is the groundtruth, expected shape: [batch_size, H, W].
        'uncertainty' is the uncertainty map, expected shape: [batch_size, H, W].
        """
        n_classes = logits.shape[1]
        logits_ = np.transpose(logits, [0, 2, 3, 1]).reshape(
            (-1, n_classes))  # flatten. Expected shape: [height*width*1, num_classes]
